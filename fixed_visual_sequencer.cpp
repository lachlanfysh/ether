...
public:
    ...
    void drawUI() {
        ...
        std::cout << BOLD "Current: " << engineColors[currentEngine] << currentEngine << ": " 
                  ...
                  << " | " << (playing ? GREEN "▶ PLAYING" : RED "⏹ STOPPED") << RESET_COLOR;
        
        if (!statusMessage.empty()) {
            std::cout << " | " << CYAN << statusMessage << RESET_COLOR;
        }
        ...
        // Help or command info
        if (showHelp) {
            std::cout << CYAN "Commands: " RESET_COLOR << std::endl;
            std::cout << "eng <0-15> | step <1-16> [note] | attack/decay/sustain/release <0-1>" << std::endl;
            std::cout << "cutoff/resonance <0-1> | play/stop | bmp <60-200> | clear | h (toggle help)" << std::endl;
        } else {
            std::cout << DIM "(type 'h' for help, 'q' to quit)" RESET_COLOR;
        }
        std::cout << std::endl;
        ...
        while (running) {
            ...
            if (command == "quit" || command == "q") {
                ...
                if (iss >> type && type >= 0 && type < MAX_ENGINES) {
                    ...
                    statusMessage = "Switched to " + std::string(name ? name : "Unknown");
                }
            }
                ...
                if (iss >> newBpm && newBpm >= 60.0f && newBpm <= 200.0f) {
                    bpm = newBpm;
                    statusMessage = "BPM set to " + std::to_string((int)newBpm);
                }
            }
    ...
    void play() {
        if (!playing) {
            playing = true;
            currentStep = 0;
            statusMessage = "Playing all engines";
            
            sequencerThread = std::thread([this]() {
                while (playing) {
                    // Only redraw if step changed
                    if (lastPlayStep != currentStep) {
                        lastPlayStep = currentStep;
                        // Update UI from this thread only when playing
                        if (playing) {
    ...
    void setParameter(const std::string& paramName, float value) {
        ...
        if (paramId >= 0) {
            ...
            ether_set_instrument_parameter(etherEngine, 0, paramId, value);
            statusMessage = paramName + " = " + std::to_string(value).substr(0, 4);
        }
    }
    ...
    void setStep(int step, int scaleNote = -1) {
        ...
        if (scaleNote == -1) {
            ...
            statusMessage = "Step " + std::to_string(step) + (enginePatterns[currentEngine][stepIndex].active ? " ON" : " OFF");
        } else {
            if (scaleNote >= 0 && scaleNote <= 15) {
                ...
                statusMessage = "Step " + std::to_string(step) + " = " + midiNoteToName(enginePatterns[currentEngine][stepIndex].note);
            }
        }
    }
    ...
    void clearPattern() {
        ...
        const char* name = ether_get_engine_type_name(currentEngine);
        statusMessage = "Cleared " + std::string(name ? name : "Unknown");
    }
    ...
    void clearAllPatterns() {
        ...
        statusMessage = "Cleared all patterns";
    }
...
int main() {
    FixedVisualSequencer sequencer;
