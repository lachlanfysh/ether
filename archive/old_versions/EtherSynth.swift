...
// MARK: - Main App (Note: Now using EtherSynthApp.swift for the @main entry point)

// MARK: - Main Content View
struct ContentView: View {
    @EnvironmentObject var synthController: SynthController
    @State private var useNewInterface = true
    ...
    var body: some View {
        ...
        .overlay(
            // Interface Toggle (for development)
            VStack {
                HStack {
                    Spacer()
                    Button(action: {
                        useNewInterface.toggle()
                    }) {
                        Text(useNewInterface ? "Legacy" : "New UI")
                            .font(.system(size: 10))
...
// MARK: - Legacy Interface View
struct LegacyInterfaceView: View {
    @EnvironmentObject var synthController: SynthController
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(["A", "A", "B", "A", "C", "B", "A"], id: \.self) { pattern in
                        PatternBlockView(pattern: pattern)
                    }
                }
                .padding(.horizontal)
            }
        ...
        VStack(spacing: 8) {
            ...
            ScrollView(.vertical) {
                VStack(spacing: 8) {
                    ...
                    ForEach(Array(synthController.getAvailableEngineTypes().enumerated()), id: \.offset) { index, engineName in
                        let engineType = index
